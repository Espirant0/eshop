...
#Теперь у нас назначен базовый обработчик исключений и, в идеале, нам теперь не требуется отслеживать исключения/ошибки.
#Они автоматом перехватятся, редиректнут на страницу ошибки и запишут ошибку в лог.
#Теперь можно оборачивать функции в более простой try-catch блок.
#Этот блок пытается выполнить функцию и, если, она не выполняется - перекидывает на страницу ошибки, записывая ошибки в лог.
#Для этого достаточно прописать ExceptionHandler::tryCatch(function(){передаваемая_функция();});
#Если всё хорошо, то никаких последствий, если же всё выполнилось с ошибкой - перекинет на страницу ошибки.
#пример синтаксиса:
\App\Service\ExceptionHandler::tryCatch(function ()
{
	intdiv(6,3); #тут нормальная функция, никаких темплейтов в чат не отправит
}
);
\App\Service\ExceptionHandler::tryCatch(function ()
{
	intdiv(2,0); #тут деление на ноль выведет подключённый темплейт для отображения страницы фатальной ошибки
}
);
#подобное поведение прямо с такими же примерами рекомендую затестировать в Application, чтобы понять, как оно работает.
#все возникающие ошибки это чудо записывает в лог (можно найти в логе по поиску TryCatch:)
